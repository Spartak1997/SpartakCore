package com.spartakcore.modctt;
...
{
  private LogHelper _mLogger = spartakcore.Logger;
  ...
  public void LoadConfig()
  {
    ...
    if( !reload() )
    {
      ...
      spartakcore.AddLoginError( "[CustomToolTips] Config file not loaded due errors" );
      InitSampleConfig();
    }
    _mInitialized = true;
  }
    ...
    if( !tPayload.isEmpty() )
    {
      ...
      if(pPlayer instanceof EntityPlayerMP)
      {
        for( CTTClientSyncMessage tMsg : tPendingMessages ) {
        	spartakcore.NW.sendTo(tMsg, (EntityPlayerMP) pPlayer);
        }
      }
      else if( pPlayer == null )
      {
        for( CTTClientSyncMessage tMsg : tPendingMessages ) {
        	spartakcore.NW.sendToAll(tMsg);
        }
      }
      else {
